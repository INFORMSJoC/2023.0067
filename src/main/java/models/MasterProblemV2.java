...
public class MasterProblemV2 extends TestableMasterProblemSplitVariablesWSR {
     ...
     * @throws IloException
     */
    public MasterProblemV2(ProductionPlanningProblemWithSalvageRevenue pp) throws IloException {
    ...
    public void addValidInequality3() throws IloException {
        for (int p = 1; p <= pp.getnProducts(); p++){
            for(int d = 1; d <= pp.getnDistributions(p); d++) {
                ...
                lhs.addTerm(1, phi[p - 1]);
                for (int f = 1; f <= pp.getnFacilities(); f++) {
                    lhs.addTerm(-pp.getSalesPrice(p) * pp.getHighestYield(p, f, d), x[p - 1][f - 1][pp.getDistributionProductionLevels(p,d,f)]);
                }
                for(int f = 1; f <= pp.getnFacilities(); f++) {
    ...
    public void addValidInequality4() throws IloException {
        for (int p = 1; p <= pp.getnProducts(); p++){
            for(int d = 1; d <= pp.getnDistributions(p); d++) {
                ...
                lhs.addTerm(1, phi[p - 1]);
                for (int f = 1; f <= pp.getnFacilities(); f++) {
                    lhs.addTerm(-pp.getLeftoverCosts(p) * pp.getHighestYield(p, f,d), x[p - 1][f - 1][pp.getDistributionProductionLevels(p,d,f)]);
                }
                for(int f = 1; f <= pp.getnFacilities(); f++) {
                    lhs.addTerm( pp.getUpperBoundProductExpectation(p), y[p - 1][f - 1][pp.getDistributionProductionLevels(p,d,f)]);
                }
                ...
                for (int d = 1; d <= pp.getnDistributions(p); d++) {
                    ...
                    if(enforced) {
                        ...
                        for (int s = 1; s <= pp.getnScenarios(p, d); s++) {
                            OptimalitySubproblemV2 optimalitySubproblem = new OptimalitySubproblemV2(p, d, s, X, pp);
